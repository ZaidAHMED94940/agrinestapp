...
var farmerProfiles = require('./farmerProfileModel');
...
const handleBuyerComplaint = async(req,res) => {
    ...
    try {
        ...
        myDoc.moveDown(1) ;
        myDoc.text(`For more details contact me on :- ${inputs.applicantContact}`) ;
        myDoc.moveDown(1) ;
...
const handleFarmerComplaint = async(req,res) => {
    ...
    try {
        const {applicantName1, applicantStreet1,applicantCity1,applicantContact1,applicantComplaint1,buyerName,policeName,policeAddress,productName1} = req.body ;
        console.log(req.body) ;

        var myDoc = new pdfFile ;
        var inputs = req.body ;

        myDoc.pipe(fs.createWriteStream('farmerComplaints.pdf')) ;
        ...
        myDoc.moveDown(1) ;
        myDoc.text(`To`) ;
        myDoc.text(`${inputs.policeName}`) ;
        myDoc.text(`${inputs.policeAddress}`) ;
        myDoc.moveDown(1) ;
        myDoc.text(`Subject : ${inputs.applicantComplaint1}`) ;
        myDoc.moveDown(1) ;
        myDoc.text(`Dear Sir , I am writing to bring to your attention a pressing issue that concerns the exploitation of local farmers by  buyers within our community.`) ;
        myDoc.text(`Farmers being subjected to exploitation by  buyer ${inputs.buyerName} who purchase agricultural products at prices significantly below the fair market rates`) ;
        ...
        myDoc.text(`They They had significantly reduced the price of item ${inputs.productName1}.`)
        myDoc.moveDown(1) ;
        myDoc.text(` I expect that you take an action as quick as possible`) ;
        myDoc.text(`For more details contact me on :- ${inputs.applicantContact1} `) ;
        myDoc.moveDown(1) ;
        myDoc.text(`Yours Faithfully`) ;
        myDoc.text(`${inputs.applicantName1}`)
        var newComplaint1 = await new farmerComplaints(
            {
                applicantName1, applicantStreet1,applicantCity1,applicantContact1,applicantComplaint1,buyerName,policeName,policeAddress,productName1
            }
        ) 
        newComplaint1.save() ;
        myDoc.end() ;
        res.download('farmerComplaints.pdf')
...
farmerComplaintRouter.post('/postFarmerComplaint',handleFarmerComplaint) ;
